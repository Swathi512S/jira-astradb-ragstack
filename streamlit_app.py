...
def vectorize_jsontxt(data):
    ...
    if data is not None:
        ...
        for issue in data:
            ...
            issue_key = issue.get("Issue Key", "Unknown")
            issue_text = json.dumps(issue, indent=4)
            texts = text_splitter.split_text(issue_text)
            texts_with_key = [ f"Issue Key: {issue_key}" + text for text in texts]
            documents = [Document(page_content=text, metadata={"source": "JIRA", "issue_key": issue_key}) for text in texts_with_key]
            count+=len(texts)
            vectorstore.add_documents(documents)
            
        st.info(f"{count} {lang_dict['load_text']}")
        

def extract_key_fields(issue_info):
    fields = issue_info.get("fields", {})
    key_fields = {
        "Issue Key": issue_info.get("key", ""),
        ...
        "Comments": [comment.get("body", "") for comment in fields.get("comment", {}).get("comments", [])] if fields.get("comment") else []
    }
    return key_fields

def load_data_from_jira(url):
        import requests
        ...
        if ":= " in credentials:
            part1, part2 = credentials.split(":=", 1) 
            credentials = part1.strip()+ ":" +part2.strip()
        
        basic_auth_token = base64.b64encode(credentials.encode()).decode('utf-8')
        ...
        if response.status_code == 200:
            try:
                ...
                all_key_fields = []
                for issue in issues:
                    key_fields = extract_key_fields(issue)
                    all_key_fields.append(key_fields)
                vectorize_jsontxt(all_key_fields)
                
            except ValueError as e:
                st.error("Failed to parse JSON response")
                st.write(response.text)  # Display raw response for debugging
        else:
            st.error(f"Failed to load data from JIRA: {response.status_code}")
...
with st.sidebar:
    ...
    # Add URL and API Key input fields here
    st.header("Load Data from JIRA")
    jira_url = st.text_input("Enter JIRA URL", key='jira_url')
    load_data_button = st.button("Load Data", key='load_jira_data_button')

#if load_data_button and jira_url and api_key:
if load_data_button and jira_url:
    st.info(f"Loading data from: {jira_url}")
    load_data_from_jira(jira_url)
...
"""
    ...
    with st.chat_message('assistant'):
        ...
        inputs = RunnableMap({
            ...
            #'issue_key': lambda x: x['issue_key']
        })
